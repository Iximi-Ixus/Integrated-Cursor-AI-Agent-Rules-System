...
if ($errorCount -gt 0) {
    Write-Host "⚠️  Warning: $errorCount cases had errors" -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "📈 File format:" -ForegroundColor Cyan
Write-Host "  Line 1: Result for private_cases.json[0]"
Write-Host "  Line 2: Result for private_cases.json[1]"
Write-Host "  Line 3: Result for private_cases.json[2]"
Write-Host "  ..."
Write-Host "  Line N: Result for private_cases.json[N-1]"

# Show first few results as verification
Write-Host ""
Write-Host "📋 First 5 results preview:" -ForegroundColor Cyan
$preview = Get-Content "private_results.txt" | Select-Object -First 5
for ($i = 0; $i -lt $preview.Length; $i++) {
    Write-Host "  Case $($i+1): $($preview[$i])"
} 